...
"""
@title CurveStableSwapNGAOracle
@author Curve.Fi
...
@dev Asset Types:
     ...
     Additional features include:
        ...
        6. The AMM contract queries Amplification factor from an external contract. This
           external contract is set by the deployer of the pool.
           Note: Users should do careful due diligence to ascertain that the pool owner address
           is not malicious/buggy.
"""
...
def _A() -> uint256:
    """
    @notice Queries an external contract for Amplification factor
    @dev Since A is in an external contract, ramping A up or down is
         a property of the external contract and not the AMM contract.
    @return uint256 Fetched amplification factor
    """
    A_oracle: uint256 = self.A_oracle
    fetched_A: uint256 = convert(
        raw_call(
            convert(A_oracle % 2**160, address),
            _abi_encode(A_oracle & ORACLE_BIT_MASK),
            max_outsize=32,
            is_static_call=True,
        ),
        uint256
...
@external
def set_A_oracle(_oracle: address, _method_id: bytes4):
    """
