...
def test_exchange(
    ...
    balance = (
        pool_tokens[0].balanceOf(bob) if pool_type == 0 else underlying_tokens[0].balanceOf(bob)
    )
...
def test_remove_liquidity(
    ...
    assert swap.balanceOf(bob) == pytest.approx(
        deposit_amounts[0] * pool_size - withdraw_amount, rel=1.5e-2
    )
    assert swap.totalSupply() == pytest.approx(
        deposit_amounts[0] * 2 * pool_size - withdraw_amount, rel=1.5e-2
    )
    ...
    if pool_type == 0:
        for i, coin in enumerate(pool_tokens):
            ...
            assert coin.balanceOf(swap) == pytest.approx(
                initial_amounts[i] - amounts[i], rel=1.5e-2
            )
    else:
