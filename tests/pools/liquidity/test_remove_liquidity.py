...
def test_remove_liquidity(
    ...
    if min_amount == 1 and (  # we specify specify min_amt_out
        ...
        swap.remove_liquidity(
            ...
            [int(0.99 * i * min_amount) for i in deposit_amounts],
            sender=alice,
        )
    else:
        swap.remove_liquidity(
            swap.balanceOf(alice), [i * min_amount for i in deposit_amounts], sender=alice
        )

    amounts_after = [coin.balanceOf(alice) for coin in coins]

    for coin, coin_type, amount_before, amount_after in zip(
        coins, pool_token_types, amounts_before, amounts_after
    ):
        ...
        if (pool_type == 0 and coin_type == 2) or (pool_type == 1 and metapool_token_type == 2):
            assert coin.balanceOf(swap) == pytest.approx(
                0,
                abs=(amount_after - amount_before)
                * (1 - 1000000 / 1000001),  # approx for rebasing tokens
            )
        else:
...
def test_remove_partial(alice, swap, pool_type, pool_tokens, underlying_tokens, pool_size):
    ...
    for coin in coins:
        assert coin.balanceOf(swap) + coin.balanceOf(alice) == pytest.approx(
            initial_amount, rel=1.5e-2
        )
